...
import json
from wiki_scraper.settings import (
...
def _get_links(response) -> set[str]:
    ...
    links_limit = random.randint(
        CRAWLER_LINKS_LOWER_THRESHOLD, CRAWLER_LINKS_UPPER_THRESHOLD
    )
    links_iter = 0
...
class WikiScraper(Spider):
    ...
    def _send_request(
        self,
        url: str,
        wait: float = REQUEST_WAIT,
        **meta_kwargs,
    ):
        self.request_counter += 1
        return SplashRequest(
            url=url, callback=self.parse, args={"wait": wait}, meta=meta_kwargs
        )

    def _log_state(self, page: str, depth: int) -> None:
        self.logger.info(
            "[Request %s][Depth %s]Current page: %s", self.request_counter, depth, page
        )
